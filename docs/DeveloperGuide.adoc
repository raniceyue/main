...
==== Implementation: Adding a new trip/activity
...
==== Implementation: Deleting a journal entry/itinerary activity
...
8. `HomeModelManager` adds the module to the `UniqueActivityList`.
9. A new folder with the `TRIP` name is created.
10. `HomeLogicManager` updates storage with updated `UniqueActivityList`.
...
The following activity diagram summarizes what happens when a user executes a add trip command:

.Activity Diagram for `add` Command
image::AddTripActivityDiagram.png[]
==== Design Considerations
